...
# 定义路径
root_dir = '/home/jinsongxi/dev/storage/'  # 更新为你的根目录
output_dir = '/home/jinsongxi/dev/storage/output_data/'  # 更新为你的输出目录
...
for i in range(1, 301):
    ...
    if len(contours) >= 2:  # 假设场景中有两个主要物体
        ...
        # 打印或保存数据
        print(f"Scene {i}: Cylinder position: {cylinder_pos}, d1: {d1}, d2: {d2}")

        # 保存每个场景的处理结果
        output_data = {
            'scene_id': i,
            'cylinder_pos': cylinder_pos.tolist(),
            'd1': d1,
            'd2': d2
        }
